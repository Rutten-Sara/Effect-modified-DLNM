...
for (k in 1:nsim){
    set.seed(10*k)
    cov_sim = rnorm(dim(shapefile_bcn)[1],0,var_true)
    
    z_sim[,k] = (cov_sim - min(cov_sim))/(max(cov_sim)-min(cov_sim))*(2*pi) # Simulate covariates
  for (i in 1:5){
    coefficients_all[[i]][,k] <- coef_base[i]*(1+coef_linear[i]*cos(z_sim[,k])) # Region-specific coefficients
  } 
    denominator_all[,k] = 2*(1+0.1*sin(z_sim[,k])) # Region-specific denominator
}
...
for (i in 1:nsim){
  ...
  suppressWarnings(y_all <- rpois(length(x),offset*exp(-8-z+cumeff+random_area))) # Simulate response
  data$y = y_all
        ...
        model_bam[[c]] = tryCatch(
          {
            model_bam <- withTimeout(
              bam(y_all ~  z_gam + crossbasis_pen + interaction_matrix + s(ID_gam, bs="re")+
                    offset(log(offset)),family=poisson(),
  ...
  # 3D plot
  library(plot3D)
  at_z = seq(0, 2*pi, length = 20)
  grid_effect_true = matrix(nrow = length(at_z), ncol = length(at_x)*(L+1))
  for (k in 1:20){
    coef_ik_grid <- coef_base*(1+coef_linear*cos(at_z[k]))
    denominator_grid <- 2*(1+0.1*sin(at_z[k]))
    grid_effect_true[k,] = trueeff(ftemp, coef_ik_grid, denominator_grid)
  }
  
  var_index <- rep(1:length(at_x), times = L+1)
  grid_effect_true_all <- sapply(1:length(at_x), function(j) rowSums(grid_effect_true[, var_index == j, drop = FALSE]))
  
  pdf("Simulation_smooth.pdf")
  persp3D(x=at_x, y=at_z, z=t(grid_effect_true_all), ticktype="detailed",theta=230,
          ltheta=200,phi=30,lphi=30,xlab="exposure",ylab="z",zlab="log-RR", zlim=c(-0.20,0.40),
          nticks = 4,cex.main = 2,
          shade = 0.75,r=sqrt(3),d=5,cex.axis=1.2, cex.lab=1.5,border=NA,
  ...
  exposure_x <- 8
  grid_effect_true_exp = matrix(nrow = length(at_z), ncol = (L+1))
  for (k in 1:20){
    coef_ik_grid <- coef_base*(1+coef_linear*cos(at_z[k]))
    denominator_grid <- 2*(1+0.1*sin(at_z[k]))
    grid_effect_true_exp[k,] = ftemp(8,0:8,coef_ik_grid,denominator_grid)
  }
  
  df_exposure <- as.data.frame(grid_effect_true_exp)
  df_exposure$row <- at_z
  df_exposure_long <- df_exposure %>%
    ...
    mutate(x = as.numeric(gsub("V", "", col)) - 1)  # x = 0:8
  
  pdf("exposure-respone_complex.pdf")
  ggplot(df_exposure_long, aes(x = x, y = exp(RR), group = row, color = row)) +
    geom_line() +
    labs(x = "lag", y = "RR", color = "z") +
    theme_minimal() +
    ggtitle("Complex")
  dev.off()
