...
# Bounding box of Barcelona (simplified)
bbox <- st_bbox(shapefile_bcn)

x_seq <- seq(bbox["xmin"], bbox["xmax"], length.out = 50)
y_seq <- seq(bbox["ymin"], bbox["ymax"], length.out = 50)
...
phi <-  0.2 * max(dist_mat)   # spatial range
sigma2 <- 1   # spatial variance
Sigma <- sigma2 * exp(-dist_mat / phi)

# Cholesky decomposition of Sigma
L <- chol(Sigma)

spat_sim_ind <- matrix(NA, nrow = nrow(shapefile_bcn), ncol = nsim)


set.seed(1)
for (i in 1:nsim) {
  z <- rnorm(n_grid)
  u_grid <- t(L) %*% z  # simulate GP
  grid$u <- u_grid
  grid_raster <- rasterFromXYZ(grid, crs = st_crs(shapefile_bcn))
  spat_sim_ind[,i] <- exact_extract(grid_raster, shapefile_bcn, 'mean')
}
...
set.seed(123)
offset_true = ceiling(rlnorm(n = 73, meanlog = log(4e5), sdlog = 1))
