...
for (i in 1:nsim){
  ...
  suppressWarnings(y_all <- rpois(length(x),offset*exp(-10.5-z+cumeff+random_area))) # Simulate response
  data$y = y_all
...
# 3D plot
library(plot3D)
at_z = seq(-1, 1, length = 20)
grid_effect_true = matrix(nrow = length(at_z), ncol = length(at_x)*(L+1))
for (k in 1:20){
  coef_ik_grid <- coef_base*(1+coef_linear*at_z[k])
  grid_effect_true[k,] = trueeff(ftemp, coef_ik_grid, 2)
}
...
for (k in 1:20){
  coef_ik_grid <- coef_base*(1+coef_linear*at_z[k])
  grid_effect_true_exp[k,] = ftemp(8,0:8,coef_ik_grid,2)
}
...
pdf("exposure-respone_linear.pdf")
